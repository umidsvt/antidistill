...
# Separate generator and evaluator models
...
OUTPUT_PATH = "train/data/limo_hint_qwen3_14b_base.json"
os.makedirs(os.path.dirname(OUTPUT_PATH), exist_ok=True)

GENERATOR_BASE_URL = "http://localhost:8001/v1"
EVALUATOR_BASE_URL = "http://localhost:8010/v1"
...
def main():
    ...
    print("START!")
    print(f"Generator: {GENERATOR_MODEL} @ {GENERATOR_BASE_URL}")
...
Using the solution above only as a reference, please re-derive the solution in a fully detailed, step-by-step manner. Include every key equation, all intermediate algebra, and clear logical justification for each step. Do not skip steps, and avoid any logical leaps or ambiguous arguments.
...
To solve the problem,""".strip()
    ...
    for batch_idx, i in enumerate(range(0, TOTAL, BATCH_SIZE)):
        ...
        while pending_indices:
            ...
            response = generator_client.completions.create(
            ...
            for local_idx, text in enumerate(texts):
                ...
                teacher_text = "To solve the problem," + text
            ...
            # Evaluate with evaluator model
            eval_response = evaluator_client.completions.create(
            ...
            # Process results
            new_pending = []
            for local_idx, (text, eval_text) in enumerate(zip(texts, eval_texts)):
                batch_idx_in_pending = pending_indices[local_idx]
                teacher_text = "To solve the problem," + text
