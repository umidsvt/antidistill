...
# Token IDs for epistemic/hedging tokens to be banned during generation
EPISTEMIC_TOKEN_IDS = [
    14524,  # 'Wait'
    14144,  # ' Wait'
    3868,   # ' wait'
...
def parse_args():
    ...
    parser.add_argument("--output_dir", default="./outputs_epistemic_prevent", type=str)
    ...
    parser.add_argument("--ban_epistemic_tokens", action="store_true", default=True,
                        help="Ban epistemic/hedging tokens like Wait, Hmm, Perhaps, etc.")
...
def infer(args):
    ...
    # Apply logit bias to suppress epistemic tokens
    logit_bias = None
    if args.ban_epistemic_tokens:
        logit_bias = {token_id: -100.0 for token_id in EPISTEMIC_TOKEN_IDS}
        print(f"Banning {len(EPISTEMIC_TOKEN_IDS)} epistemic tokens")
    ...
    # Generate responses across multiple epochs
    file_outputs = []
    correct_cnt = 0
    ...
    # Evaluate correctness and compute pass@k
    pass_at_k_list = []
    k = args.k
